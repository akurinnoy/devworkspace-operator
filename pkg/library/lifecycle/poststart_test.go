...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Using assert.Equal after trimming whitespace might be too brittle if the
			// exact formatting of the template string changes slightly (e.g. an extra newline).
			// For more robust testing, one might check for specific substrings or use
			// a more sophisticated script comparison.
			// However, for now, we'll compare the exact expected string.
			assert.Equal(t, tt.expectedScript, script)
		})
	}
}
